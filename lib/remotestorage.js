...
loadData();
if(config.https) {
  require('https').createServer(config.https, serve).listen(config.port1);
  require('https').createServer(config.https, serve).listen(config.port2);
} else {
  require('http').createServer(serve).listen(config.port1);
  require('http').createServer(serve).listen(config.port2);
}
